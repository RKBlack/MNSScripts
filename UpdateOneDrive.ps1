...
Write-Host "Initialize OneDriveSetup with update argument..."
$OneDriveSetup = (Join-Path "$($env:TEMP)" "OneDriveSetup.exe")

Write-Host "Now time to install OneDrive in program folder $($OneDriveSetup) /update"
$proc = Start-Process -FilePath $OneDriveSetup -ArgumentList "/update" -WindowStyle Hidden -PassThru
$proc.WaitForExit()
